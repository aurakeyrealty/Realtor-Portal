# Aura Key — Realtor Portal

Guide for anyone (human or agent) changing this repo. Everything here is
non-obvious: if a fact can be read off the code in ten seconds, it is not
repeated here.
...
Adjectives are not enforceable. Every rule below has a **trigger** (when it
fires) and a **check** (how anyone can tell it was ignored).

### Before writing anything

- **Search before you write.** Before adding a function, helper, style or
  screen, grep for the thing it would do. In your reply, either name the
  existing thing you are reusing, or say what you searched for and that nothing
  matched. "I did not find one" is an acceptable answer; not looking is not.
- **Before creating a file.** There are five server files and three HTML files,
  each with a stated remit (§2). A change belongs in one of them unless you can
  name why none fits. A new root file also needs a `.claspignore` line or
  verification fails (§3.2).
- **Before adding a dependency, an abstraction, a config flag or an options
  object.** Don't. The app has zero dependencies and no build step by design.
  If you think one is warranted, that is a design decision — see below.
...
- **Smallest change that fully solves the problem.** No speculative parameters,
  ...
  unrelated problem is useful — say so, don't fix it in the same change.
- **Reuse the seams that exist:** `call()`, `esc()`, `safeUrl()`,
  `cachePut_`/`cacheGet_`, `readTab_`, `requireAuth_`, the `LOAD` and `NAV`
  tables, the CSS tokens in `:root`. A new helper that duplicates one of these
  is a defect, not a preference.
- **Match the file you are in** (§6). Style is the file's, not yours.

### Record why, not just what

- **Append an entry to `docs/worklog.md`, newest first, in the same change.**
  Trigger: any change a future reader could reasonably want to undo — a design
  decision, a rejected alternative, a non-obvious constraint, a bug whose fix
  looks arbitrary without its story. Check: `git log` shows a substantive commit
  with no matching worklog entry.
...
- Write the **reason**, not the diff. The code says what changed and `git log`
  says when. The worklog is for what neither records: the option you rejected and
  why, the constraint that forced the shape, what it will cost to reverse.
- Sharp edges belong in **both** places — a comment where the trap is, so nobody
  meets it unwarned, and the worklog entry so the next person understands the
  shape of the whole change.

### Tests

The portal has no unit-test suite; **`dev/verify.mjs` is the gate**, and it
loads the real files through the real code paths.

- "Add a test" means **add a check to `dev/verify.mjs`**. For `aura-chat/`, it
  means `pytest` under `aura-chat/tests/`.
- **Fixing a bug that verification could have caught? Add the check that would
  have caught it, in the same change.** Precedent: the stray-root-file check
  exists because a design handoff folder nearly got pushed as server code.
- Changing something a check asserts — the dispatch table, `include()` targets,
  the cache chunker, column-scoped reads — means updating that check in the
  same change, not after.
- Run `node dev/verify.mjs` before claiming done, and report the result.

### Design decisions belong to the user

**Stop and ask** when the change involves: sheet or schema shape, a new action
in the dispatch table, anything touching auth or `PUBLIC_ACTIONS`, what a new
screen should do or look like, a new external service or dependency, or two
defensible approaches with materially different trade-offs. Present the options
and the trade-off, then wait.

**Do not ask** about naming, formatting, which existing helper to use, or
anything §1–§6 already answers. Decide those and move on.
...
## 8. How the rules above are enforced

Prose is the weak layer. The mechanical layer:

| Rule | Mechanism |
|---|---|
| No stray file pushed as server code | `dev/verify.mjs` fails on any unignored root path |
| Dispatch table stays wired | `dev/verify.mjs` resolves every `case` to a real function |
| No undefined helper ships | `dev/verify.mjs` cross-checks every `foo_()` call |
| Server files still load together | `dev/verify.mjs` runs all of them in one VM context |
| Bundle never goes stale | `dev/deploy.mjs` rebuilds before every deploy |
| Verification actually runs | A `Stop` hook in `.claude/settings.json` running `node dev/verify.mjs` |
| Simplicity and reuse | `/code-review` and `/simplify` before committing |
| Reasons survive the author | `docs/worklog.md`, appended in the same change (§7) |

If an agent gets the same thing wrong twice, that is a signal to add a check to
`dev/verify.mjs` — not to add another sentence to this file.

---

## 9. `aura-chat/` (separate service)

Python 3.12 + FastAPI, planned for Railway, streaming an LLM agent over SSE.
It is a **separate service that reuses the portal's API and auth** — it holds no
service account, forwards the realtor's own token, and re-verifies it with the
same `TOKEN_SECRET`. Structured ports-and-adapters: `app/domain` (source-agnostic
models), `app/ports` (Protocols), `app/adapters` (one implementation each).
Tooling is `ruff` (line length 100) and `pytest` (asyncio auto mode).

Design and rationale: [`docs/aura-chat/architecture.md`](docs/aura-chat/architecture.md).
Discovery it rests on: [`docs/aura-chat/investigation-aur-3-4-5.md`](docs/aura-chat/investigation-aur-3-4-5.md).
